...
- UI and ranking logic began in Streamlit + Python.
- Dataset originally loaded via KaggleHub or manual CSV upload.
- Runtime originally assumed a local-machine workflow.

## Current Operational State
- Public lighter lane is hosted on Cloud Run:
  - `https://studio.djmixingstation.com`
- Developer/debug lane is hosted separately on Cloud Run:
  - `https://dev.djmixingstation.com`
- Production app reads the promoted snapshot contract from managed storage.
- Scheduler and retention jobs run through GitHub Actions + GCS + OIDC.
- App deploys, rollback drills, smoke checks, and staged capacity checks are operational.

## Target State (Live)
- Public URL with managed hosting, uptime, and separate public/debug lanes as needed.
...
## Current Delivery Status (as of 2026-03-24)
...
- Phase 2 data productization is operational:
  - versioned snapshots are published to managed storage
  - promoted `latest.json` is the runtime contract
  - rollback to previous dataset version is possible
- Phase 3 hosting and delivery are operational for the current scope:
  - Cloud Run hosting is live
  - public lighter lane is live on `studio.djmixingstation.com`
  - developer/debug lane is live on `dev.djmixingstation.com`
  - per-release deploy pipeline is working
  - TLS/domain routing is active
  - rollback path is verified
- Current next focus:
  - Sunday soft-launch execution readiness
  - minimum `studio` product event layer
  - first-pass launch product dashboard
  - optional higher-rung public-lane stress testing before Phase 6
...
- launch-focused product analytics plan now lives in:
  - `recommendation_app/go-live/plan/43_soft_launch_product_analytics_schema.md`
  - `recommendation_app/go-live/plan/44_soft_launch_product_metrics_dashboard_plan.md`
...
- Containerize app.
- Deploy to managed runtime (Cloud Run for production).
- Add CDN/edge TLS and domain.
- Add per-release deployment pipeline.
- Split the hosted app into:
  - public lighter lane for users
  - developer/debug lane for richer diagnostics
- Validate public smoke and staged capacity checks before treating the public lane as stable.

Exit criteria:
- Public URL works for anonymous users.
- Blue/green or quick rollback path is verified.
- Public user lane passes smoke and staged capacity checks.

Developer/debug lane practical completion for the current team size:
- lane stays separate from the public user lane
- page opens reliably
- debug-lane smoke passes
- core debug sections are reachable
- no immediate hydration/reset glitches appear
- main debug views do not glitch or reset during normal solo use
- one manual sanity pass feels smooth
- treat that as enough for a 1-2 developer lane
- do not add a debug-lane concurrency ladder unless real usage shows the need
- do not treat debug-lane smoothness as a production gate for public release work
